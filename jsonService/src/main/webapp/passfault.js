...
passfault.protection = {
  ...
  // },
  WPA: {
    display: "Wifi Password (WPA Password Hashing)",
    cracker: "SHA1",
    iterations: 4096
  }
};
passfault.loadSelectProtection = function(select) {
  ...
  for (var type in this.protection) {
    if (this.policy.algorithm && this.policy.algorithm == type) {
      htmlToInsert +=
        '<option value="' + type + '" selected>' + passfault.protection[type].display + "</option>";
    } else {
      htmlToInsert +=
        '<option value="' + type + '" >' + passfault.protection[type].display + "</option>";
    }
  }
  select.html(htmlToInsert);
}ebug2: channel 0: window 998924 sent adjust 49652
...
passfault.attacker = {
  ...
  "500K": {
    display: "The future of things to come",
    multiplier: 1000
  }
};
...
passfault.loadSelectCracker = function(select) {
  ...
  for (var cracker in this.attacker) {
    if (this.policy.cracker && this.policy.cracker == cracker) {
      htmlToInsert +=
        '<option value="' + cracker + '" selected>' + this.attacker[cracker].display + "</option>";
    } else {
      htmlToInsert +=
        '<option value="' + cracker + '" >' + this.attacker[cracker].display + "</option>";
    }
  }
  select.html(htmlToInsert);
};
...
passfault.patternDescription = {
  LEET:
    "A word was found with some 'leet substituted characters.  Leet is short for \"elite\".  It is a way of encoding characters with numbers and special characters. Such as 1 for the letter 'l', for example, leet is spelled 133+.",
  ...
  WORD:
    "A word was found.  To add more complexity, consider misspelling a word or inserting characters into a word",
  INSERTION: "A word was found with some extra characters inserted.",
  DUPLICATE: "The same pattern was found previously.",
  DIAGONAL:
    "A diagonal sequence of keys was identified.  This is the weakest type of password pattern.",
  HORIZONTAL:
    "A horizontal sequence of keys was identified.  This is the weakest type of password pattern. ",
  REPEATED: "Repeat characters were found.  This is an extremly weak type of password pattern.  ",
  RANDOM:
    "No pattern was found.  This part of the pattern increases in strength by using many different types of characters, such as upper-case, lower-case, latin, cyrillic, numbers, or other special characers.",
    BACKWARDS: "A word was found that was typed backwords."
};
...
passfault.passwordPatternTemplate =
  ...
  '<div class="normal"><label title="Found Pattern"/>Potential {name}</div>' +
...
passfault.applyTemplate = function(div, analysis, passlength) {
  ...
  var timeToCrackMilliseconds = this.timeToCrackMilliseconds(
    analysis.cost,
    $("#attacker").val(),
    $("#hasher").val()
  );
  ...
  for (var pIndex = 0, len = analysis.patterns.length; pIndex < len; pIndex++) {
    ...
    if (pattern.classification == "null") pattern.classification = "&emsp;";
  ...
  for (pIndex = 0, len = analysis.patterns.length; pIndex < len; pIndex++) {
    ...
    cleanPattern = pattern.name.split(" ")[0];
    pattern.detail = this.patternDescription[cleanPattern];
...
passfault.submitPassword = function() {
  //submit to applet or service
  var applet = false;
